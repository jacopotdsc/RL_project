...
class Net(nn.Module):
    def __init__(self,  env1_id, env1_outputs, 
                        env2_id, env2_outputs, 
                        env3_id, env3_outputs, 
        ...
        # hidden layers   
        self.hl1    = nn.Linear(in_features=self.encoder.size, out_features=self.hidden_size, bias=bias) # 100 ---> 64
        self.hl_fin = nn.Linear(in_features=self.hidden_size, out_features=self.hidden_size, bias=bias) # 64 ---> 64


        # output layers: one for each enviroment
        self.output_env1 = nn.Linear(in_features=self.hidden_size, out_features=self.env1_outputs, bias=bias) # 64 ---> output
    ...
    # return the q-value -> input = ( encoded_x, env_id )
